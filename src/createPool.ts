...
function showConfigBanner() {
  ...
  logger.error('*****************************************************************************************************');
  logger.error(
    'PGHOST: Database host | PGUSER: Database login username | PGPASSWORD: Database password | PGDATABASE: Database name | PGPORT: Database port',
  );
...
export function createPool(opts: DbConfiguration = {}): Pool {
  ...
  if (!maxConnections)
    logger.info(
      `Connection pool length is not set. Please export PGMAX_CONNECTIONS environment variable. Defaults to ${maxConnections}`,
    );
